  ...
  function filter(albums) {
    const filteredData = albums.filter(
      (album) =>
        ...
        album.artistName.toLowerCase().includes(searchData.toLowerCase())
    );
    return filteredData;
  }
  ...
  return (
    ...
    >
      <Flex flexDirection="column" justifyContent="flex-start">
        <Header setSearchData={setSearchData} filterAlbums={filterAlbums} />
        <Container>
          <Flex
          ...
          >
            {searchData.length === 0 &&
              albumsKeys?.map((key, i) => {
                return (
                  <Box width="100%">
                    <Carousel songs={albums[key]} genre={key} key={i} />
                  </Box>
                );
              })}
          </Flex>
